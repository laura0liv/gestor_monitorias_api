...
class MonitorMateriaController:
    ...
    def get_monitor_materia(self, id_monitor: int, id_materia: int):
        ...
        try:
            ...
            cursor.execute("""
                SELECT id_monitor, id_materia
                FROM monitor_materia
                WHERE id_monitor = %s AND id_materia = %s
    ...
    def create_monitor_materia(self, monitor_materia_data: dict):
        ...
        try:
            ...
            result = cursor.fetchone()
            if result is None:
                return {"message": "La asignación ya existe"}
            conn.commit()
            return {
    ...
    def get_materias_by_monitor(self, id_monitor: int):
        ...
        try:
            ...
            return {"id_monitor": id_monitor, "materias": results}
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
    ...
    # GET /monitores/por_materia/{id_materia}
    #
    # Retorna los monitores activos asignados a la materia,
    # con su calificación promedio de monitorías completadas.
    # Campos: id_usuario, nombre, apellido, promedio_calificacion
    # ─────────────────────────────────────────────────────────

    def get_monitores_por_materia(self, id_materia: int):
        conn = None
        ...
        try:
            ...
            cursor.execute("""
                SELECT
                    ...
                    u.nombre,
                    u.apellido,
                    ROUND(AVG(c.puntuacion)::numeric, 1) AS promedio_calificacion
                FROM monitor_materia mm
                ...
                LEFT JOIN monitoria mon
                    ON mon.id_monitor = u.id_usuario
                    AND mon.id_materia = mm.id_materia
                    AND mon.estado = 'Completada'

                LEFT JOIN calificacion c
                    ON c.id_monitoria = mon.id_monitoria
            ...
            if not result:
                raise HTTPException(
                    status_code=404,
                    detail="No hay monitores disponibles para esta materia"
                )
